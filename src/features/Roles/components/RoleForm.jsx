// import ScreenPermissions from "../../Permissions/ScreenPermissions"
// function RoleForm() {
//   return (
//     <div>
//       <ScreenPermissions/>
...
import { createRole, updateRole } from "../RolesThunks";
// import SuccessAlert from "../../globalComponents/Alerts/SuccessAlert";
import SuccessAlert from "../../../globalComponents/Alerts/SuccessAlert"; 
function RoleForm({ mode = "create", initialData = null, onSuccess, onCancel }) {
  ...
  useEffect(() => {
    if (isEditMode && initialData) {
      ...
      // Set permissions from initialData
      if (initialData.permissionIds && initialData.permissionIds.length > 0) {
        initialData.permissionIds.forEach(permissionId => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      console.log(`${isEditMode ? "Updating" : "Creating"} role with data:`, formattedData);
      ...
      console.log(`Role ${isEditMode ? "updated" : "created"} successfully:`, result);
    } catch (error) {
  ...
  const handleReset = () => {
    ...
    if (isEditMode && initialData) {
      ...
      screenPermissions.forEach(permission => {
        const wasSelected = initialData.permissionIds?.includes(permission.id);
        setValue(`permission_${permission.id}`, wasSelected || false);
      });
    } else {
